...
const Header = () => {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 w-full flex justify-center items-center md:pt-[1.6%] z-50">
      {/* PC Menu */}
      <div className="hidden md:flex w-[60%] justify-between items-start gap-3">
        {/* Logo Placeholder */}
        <img src="/images/logo-zica.png" alt="logo" className="w-[13%] object-contain" />
        {/* Desktop Navigation */}
        <nav className="hidden md:grid md:grid-cols-5 w-full bg-gradient-to-b from-[#031F60] to-[#000839] shadow-lg rounded-[10px] border-4 border-[#354573] justify-between items-center px-[1%] pt-[1%] pb-[1.5%] text-lg md:text-xl font-extrabold text-white">
          {navLinks.map((link, idx) => (
            <div key={link.href} className={`flex items-center relative${idx === navLinks.length - 1 ? " justify-center" : ""}`}> 
              {link.href === "/cong-dong" ? (
                <>
                ...
                {dropdownOpen && (
                  <div className="absolute top-full left-0 mt-2 w-full bg-[#FAFBA1] border rounded-xl shadow-lg z-50">
              ...
              {idx !== navLinks.length - 1 && <div className="mx-[5%] w-[2px] h-10 bg-[#FFFFFF]/50"></div>}
            </div>
          ))}
        </nav>
      </div>
      ...
      {/* Mobile Menu Button */}
      <div className="md:hidden w-full h-[23.2%] bg-[#031C5B] flex justify-between items-center relative border-4 border-[#343A61] rounded-b-xl p-4">
        <img src="/images/logo-zica.png" alt="logo" className="w-[9.6%] object-contain" />
        <button className="w-[25%] h-[36px] px-2 py-3 bg-[linear-gradient(180deg,_#A9FC00_36.06%,_#03E75A_63.94%)] shadow-[inset_0px_9px_4px_#B6F309,_inset_0px_-9px_4px_rgba(182, 243, 9, 0.9)] border-[3px] border-[#37CE26] 
        text-sm font-extrabold text-[#010B35] rounded-full flex items-center justify-center">Chơi ngay</button>
        <button className="w-[25%] h-[36px] px-2 py-3 bg-[linear-gradient(180deg,_#5DF9FA_36.06%,_#54C3FB_63.94%)] shadow-[inset_0px_9px_4px_#63FCFA,_inset_0px_-9px_4px_rgba(99, 252, 250, 0.9)] border-[3px] border-[#375BF4] 
        text-sm font-extrabold text-[#010A35] rounded-full flex items-center justify-center">Nạp thẻ</button>
